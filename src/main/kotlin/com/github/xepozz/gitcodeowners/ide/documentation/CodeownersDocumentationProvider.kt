...
//        return listOf(CodeownersGuruUtils.generateCrontabGuruUrl(element))
    ...
    override fun getCustomDocumentationElement(
        ...
        if (file !is CodeownersFile) return null

        return PsiTreeUtil.findFirstParent(contextElement) { it is CodeownersDefinition }
    }
...
//            println("Markdown converter not available, returning plain text.")
    ...
    override fun collectDocComments(file: PsiFile, sink: Consumer<in PsiDocCommentBase>) {
        buildList {
            addAll(PsiTreeUtil.findChildrenOfType(file, CodeownersDefinition::class.java))
        }
            .forEach { expression ->
//                println("comments for $expression")
                val comment = CodeownersDocumentationUtils.findContextualDocumentationElement(expression) ?: return@forEach
